    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{t.metrics.totalSessions}</p>
          <p className="mt-2 text-2xl font-semibold text-zinc-900 dark:text-zinc-100">{summaryMetrics.totalSessions}</p>
        </div>
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{t.metrics.submitted}</p>
          <p className="mt-2 text-2xl font-semibold text-sky-700 dark:text-sky-300">{summaryMetrics.trainerSubmitted}</p>
        </div>
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{t.metrics.published}</p>
          <p className="mt-2 text-2xl font-semibold text-emerald-700 dark:text-emerald-300">{summaryMetrics.published}</p>
        </div>
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{t.metrics.participants}</p>
          <p className="mt-2 text-2xl font-semibold text-zinc-900 dark:text-zinc-100">{summaryMetrics.participants}</p>
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                      {new Date(session.start_date_time).toLocaleString(isArabic ? 'ar-OM' : 'en-OM')}
                    </p>
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        {new Date(selectedSession.start_date_time).toLocaleString(isArabic ? 'ar-OM' : 'en-OM')}
                      </p>
                    </div>
                  </div>
                                ...
                                {selectedSessionDetails.bookings.slice(0, 10).map((booking) => (
                                  <tr key={booking.id} className="border-t border-zinc-200 dark:border-zinc-800">
                                    ...
                                    <td className="py-2 text-zinc-500 dark:text-zinc-400">
                                      {new Date(booking.created_at).toLocaleDateString(isArabic ? 'ar-OM' : 'en-OM')}
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                            ...
                            {selectedSessionDetails.final_recipe_published_at
                              ? new Date(selectedSessionDetails.final_recipe_published_at).toLocaleString(isArabic ? 'ar-OM' : 'en-OM')
                              : '-'}
                          </p>
                        </div>
                        <input
                              ...
                              >
                                <div className="flex items-center justify-between gap-3 text-xs text-zinc-500 dark:text-zinc-400">
                                  ...
                                  <span>
                                    ...
                                    {new Date(feedback.created_at).toLocaleDateString(isArabic ? 'ar-OM' : 'en-OM')} • ★ {feedback.rating}
                                  </span>
                                </div>
